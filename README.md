...
```mermaid
graph TD
    User[Ops Analyst] -->|HTTPS| FE[React 19 + Vite<br/>nginx-alpine]

    FE -->|/api/* proxy| BE[Spring Boot 3<br/>Java 25<br/>JWT + Actuator]

    BE -->|JDBC| PG[(PostgreSQL 16<br/>Liquibase)]

    BE -->|KafkaTemplate| K[Apache Kafka<br/>trade-events<br/>recon-results<br/>system-alerts<br/>trade-events-dlq]
...
graph LR
    ...
    GH -->|trigger workflow| CI[GitHub Actions<br/>lint → test → coverage → docker build]

    CI -->|push images| GHCR[GHCR<br/>Docker Images]

    GHCR -->|docker compose pull| LAP[Demo Laptop]

    LAP -->|docker compose up -d| STACK[ReconX 7-Service Stack<br/>Backend + Frontend + Kafka + DB + Monitoring]
```
